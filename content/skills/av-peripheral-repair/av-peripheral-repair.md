...
metadata:
  ...
  pill:
    ...
    order: 13
sourceUrl: https://github.com/idemeum/skills/blob/main/av-peripheral-repair/SKILL.md
---
...
Do NOT use this skill when the symptom is collab-app-specific — e.g. "Zoom can't see my mic but other apps can" — use [collab-app-repair](../collab-app-repair/collab-app-repair.md) instead. Cross-app A/V symptoms (the OS doesn't see the device at all) belong here.
...
- The expected device IS listed but is not the default → a setting issue, not a hardware issue. The user can change the default via System Settings → Sound (macOS) or Settings → System → Sound (Windows). For per-app overrides, use [collab-app-repair](../collab-app-repair/collab-app-repair.md)
...
- Camera in list but not selected by an app → app-specific configuration; route to [collab-app-repair](../collab-app-repair/collab-app-repair.md)
